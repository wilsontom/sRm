...
setMethod('indexRT', signature = 'SRM',
          function(object, index, sampleName) {
            ...
            local_max_rt <-
              round(chrom_tibble$rt[MsCoreUtils::localMaxima(chrom_tibble$int, hws = 40)], digits = 2)

            local_max_int <-
              round(chrom_tibble$int[MsCoreUtils::localMaxima(chrom_tibble$int, hws = 40)], digits = 2) /
              sum(chrom_tibble$int) * 100

            local_max_tib <- tibble::tibble(rt = local_max_rt, int = local_max_int) %>%
              dplyr::filter(int >= 5.0)

              return(local_max_tib$rt)
