...
`sRm` is designed to make use of existing packages (ie, much of the heavy lifting is done by `MSnbase` and `xcms`) and create a simple S4 interface to the user for processing, visualisation and reporting. 
 ...
 * **header**
    * a `tibble` of formatted header data.
...
group_table_filtered <-
  ...
  dplyr::filter(GroupWidth <= 3) %>% dplyr::filter(count <= number_of_samples)
...
```{r, createGroup}
srmExpGroupTarg <- createGroup(srmExpPeaks, index = 1, rt = 5.0,  width = 30, id = 'Group01')
